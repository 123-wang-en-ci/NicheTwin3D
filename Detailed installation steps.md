...
## V. Extract the Windows Frontend App

You should already have the Windows frontend package `NicheTwin3D_Windows(1).zip`. **Do not run the `.exe` directly inside the ZIP file; it must be extracted first.**

**Steps:**
1. Locate `NicheTwin3D_Windows(1).zip` in your File Explorer.
...
## VI. Download `data.zip`

This step is critical. The GitHub source code does not include the model weights and H5AD data files. You need to download `data.zip` from Hugging Face, extract it, and place the model weights and dataset into the `Server` directory.
