...
# H5AD The file stores the column names of the real labels
...
# High-level downstream task classifier head
...
def train_classifier(features, labels, save_name, device, save_dir):
    print(f"\nStarting training classifier: {save_name}")
    
    le = LabelEncoder()
    targets = le.fit_transform(labels)
    num_classes = len(le.classes_)
    print(f"Found {num_classes} classes: {le.classes_[:5]}...")
    ...
    # Automatically infers input_dim from feature dimensions to prevent mismatches
...
def main():
    ...
    if CELL_TYPE_COL in engine.adata.obs:
        ...
        print(f"Original cells: {n_total}, Kept: {n_keep} (Removed {n_total - n_keep} ambiguous cells)")
        if n_keep > 0:
            train_classifier(embeddings[valid_mask], raw_labels[valid_mask].values,
                             "cell_type_model", engine.device, current_dir)
        else:
            print("Error: No remaining cells after filtering, please check the filtering criteria!")
