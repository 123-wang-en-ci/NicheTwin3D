...
class SpatialNicheDataset(Dataset):
    def __init__(self, h5ad_path, context_length=1024, n_neighbors=20):
        ...
        print(f"[Training] The gene vocabulary has been saved to: {vocab_path} (Len: {len(self.gene_names)})")

        self.start_idx = 8
        self.gene_to_id = {gene: i + self.start_idx for i, gene in enumerate(self.gene_names)}
        self.n_tokens = len(self.gene_names) + 20 
        ...
        print(f"The data set is ready. Number of cells: {self.adata.n_obs}")
...
def train():
    ...
    print(f"Trainer (Epochs: {MAX_EPOCHS}, Acc: {accelerator})...")
    ...
    if len(ckpt_files) > 0:
        ...
        print(f"Previous training breakpoints detected, resuming training from {latest_ckpt}...")
    ...
    print(f"Training completed, total time: {int(hours)}hours {int(minutes)}minutes {int(seconds)}seconds")
    ...
    print("Weights saved successfully! You can now run evaluate_model.py.")
