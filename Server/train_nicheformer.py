...
class SpatialNicheDataset(Dataset):
    def __init__(self, h5ad_path, context_length=1024, n_neighbors=20):
        ...
        # Save the word list locally for server.py and model_engine.py to read!
        vocab_path = os.path.join(current_dir, "gene_vocab.npy")
        np.save(vocab_path, self.gene_names)
        print(f" [Training] The gene glossary has been saved to: {vocab_path} (Len: {len(self.gene_names)})")
        self
...
def train():
    H5AD_PATH = "Allen2022Molecular_lps_MsBrainAgingSpatialDonor_14_1.h5ad" 
    ...
    if not os.path.exists(H5AD_PATH):
        print(f"The data file cannot be found {H5AD_PATH}")
        return
    ...
    print(f"The training is complete and the weights are being saved to  {OUTPUT_PATH} ...")
