...
class DataManager:
    ...
    def load_and_sync_data(self):
        ...
        if target_debug_id in self.adata.var_names:
            ...
            if not found_in_col:
                print(f"{target_debug_id} is not found in the entire data table, probably the data filtered out the gene.")
        print("="*40 + "\n")

        if 'counts' not in self.adata.layers:
            self.adata.layers['counts'] = self.adata.X.copy()

        print("[Nicheformer] Sync data to AI engines...")
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                print("[System] The Nicheformer weights load successfully")
            except Exception as e:
                print(f"[Warning] Nicheformer fails to load: {e}")
        else:
            print(f"[Warning] No weight file found: {self.model_path}, will be run with the untrained model (test flow only). ")
    ...
    def update_clusters(self, cluster_ids, legend_info):
        """
        Save the AI-generated clustering results to adata.obs 
        so that they can be written to the hard disk when you click the "Save" button later.
        """
    ...
    def impute_data(self, gene_values):
        """
        Calling Nicheformer for Gene Expression Interpolation Note: The gene_values parameter may not be used here, as Nicheformer is reconstructed from hidden space
        """
    ...
    def save_zero_shot_result(self):
        ...
        try:
            ...
            try:
                # Parsing JSON: [{'id':0, 'name':'Cluster 0', 'color':'#aabbcc'}, ...]
            ...
            # The path falls back two levels to Unity's StreamingAssets
            save_path = os.path.join(self.base_dir, "..", "Assets", "StreamingAssets", filename)
