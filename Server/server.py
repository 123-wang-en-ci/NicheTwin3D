...
class DataManager:
    def __init__(self):
        ...
        print("[System] Initialize the Nicheformer engine...")
    ...
    def load_and_sync_data(self):
        ...
        print(f"Total gene number: {self.adata.n_vars}")
        ...
        if target_debug_id in self.adata.var_names:
            ...
            print(f"Target gene {target_debug_id} is not in the index!")
            
            found_in_col = False
            for col in self.adata.var.columns:
                # 检查这一列是否包含该 ID
                if self.adata.var[col].astype(str).str.contains(target_debug_id).any():
                    print(f"Found {target_debug_id} present in column '{col}', not in the index.")
                    print(f" (The frontend sends the ID, but the model currently uses an index other than '{col}')")
                    found_in_col = True
            
            if not found_in_col:
                print(f"Not found in the entire datasheet {target_debug_id}，It may be that the data filtered out the gene。")
        print("="*40 + "\n")
        if 'counts' not in self.adata.layers:
            self.adata.layers['counts'] = self.adata.X.copy()

        print("[Nicheformer] Synchronize data to the AI engine...")
        ...
        print(f"[Debugging] The engine mapping table has been constructed and contains the {len(self.ai_engine.gene_to_id)} genes。")
        ...
        if os.path.exists(self.model_path):
            ...
            print(f"[Warning] Weight file not found: {self.model_path}，Will be run with an untrained model (test flow only)。")
    ...
    def export_csv_for_unity(self):
        print("[Sync] Generate CSV for Unity (perform coordinate centralization)...")
    ...
    def impute_data(self, gene_values):
        ...
        try:
            ...
            if imp_p99 > 1e-8:
                ...
                imputed_vals = imputed_vals * scale_factor
            ...
            return np.clip(imputed_vals, 0.0, 5.0)
    ...
    def save_region_result(self):
        print("[Save] Save the results of the organization area segmentation...")
        try:
